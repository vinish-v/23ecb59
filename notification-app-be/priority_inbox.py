...
# Numeric weight ranking: placement > result > event
WEIGHT_RULES = {
    "Placement": 3,
    "Result": 2,
    "Event": 1
...
class NotificationItem:
    ...
    def __lt__(self, other):
        # We need a min-heap to preserve the top elements.
        # Elements will be popped out if they have lower weight or older timestamp.
...
class PriorityInboxTracker:
    ...
    def insert(self, item: NotificationItem):
        if len(self.heap) < self.limit:
            ...
            # If the current incoming notification is higher priority than
            # the absolute minimum priority item in the heap, replace it.
            if item > self.heap[0]:
                heapq.heappushpop(self.heap, item)

    def retrieve_ordered(self) -> list:
        # Heap elements sorted descending to give top priority list first
